...
impl PersonQuery {
    ...
    pub fn run(&self, db: &Database) -> KinforgeResult<Vec<Person>> {
        ...
        Ok(candidates.into_iter().filter(|p| self.matches(p)).collect())
    }
...
mod tests {
    ...
    fn test_person_query_surname() {
        let db = setup();
        let results = PersonQuery::new()
            .surname_contains("Anderson")
            .run(&db)
            .unwrap();
    ...
    fn test_source_query() {
        let db = Database::open_in_memory().unwrap();
        let mut s1 = Source::new("1900 US Census");
        s1.year = Some(1900);
        let mut s2 = Source::new("1910 US Census");
        s2.year = Some(1910);
        let mut s3 = Source::new("Birth Register");
        s3.year = Some(1850);
        db.insert_source(&s1).unwrap();
        db.insert_source(&s2).unwrap();
        db.insert_source(&s3).unwrap();

        let results = SourceQuery::new()
            .title_contains("census")
            .run(&db)
            .unwrap();
        assert_eq!(results.len(), 2);
