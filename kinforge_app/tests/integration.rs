...
fn add_and_get_person() {
    let a = app();
    let p = a.add_person(Some("Mary"), Some("Jones"), Sex::Female, None).unwrap();
...
fn update_person_name() {
    let a = app();
    let mut p = a.add_person(Some("Jim"), Some("Brown"), Sex::Male, None).unwrap();
...
fn delete_person_cascades_events() {
    let a = app();
    let p = a.add_person(Some("Bob"), Some("White"), Sex::Male, None).unwrap();
    let e = a.add_event(p.id.clone(), EventType::Birth, None, None, None).unwrap();
...
fn add_alternate_name() {
    let a = app();
    let p = a.add_person(Some("Elizabeth"), Some("Windsor"), Sex::Female, None).unwrap();
    let p2 = a.add_name_to_person(&p.id, None, Some("Mountbatten-Windsor"), NameType::Birth).unwrap();
...
fn add_event_with_date_and_place() {
    ...
    let e = a.add_event(p.id.clone(), EventType::Birth, Some(date.clone()), Some("Boston, MA"), None).unwrap();
...
fn update_event() {
    ...
    let mut e = a.add_event(p.id.clone(), EventType::Residence, None, None, None).unwrap();
...
fn delete_event() {
    ...
    let e = a.add_event(p.id.clone(), EventType::Census, None, None, None).unwrap();
...
fn event_date_between_validation() {
    ...
    let result = a.add_event(p.id.clone(), EventType::Birth, Some(EventDate::Between(d1, d2)), None, None);
...
fn add_family() {
    let a = app();
    let father = a.add_person(Some("Fred"), Some("Smith"), Sex::Male, None).unwrap();
    let mother = a.add_person(Some("Greta"), Some("Smith"), Sex::Female, None).unwrap();
    let child = a.add_person(Some("Henry"), Some("Smith"), Sex::Male, None).unwrap();

    let spouse = a.add_relationship(RelationshipType::Spouse, father.id.clone(), mother.id.clone(), None).unwrap();
    let pc1 = a.add_relationship(RelationshipType::ParentChild, father.id.clone(), child.id.clone(), None).unwrap();
    let pc2 = a.add_relationship(RelationshipType::ParentChild, mother.id.clone(), child.id.clone(), None).unwrap();
...
fn source_crud() {
    let a = app();
    let src = a.add_source("1900 Census", Some("US Gov"), None, Some(1900), None, None).unwrap();
...
fn citation_links_source_to_event() {
    ...
    let e = a.add_event(p.id.clone(), EventType::Birth, None, None, None).unwrap();
    let s = a.add_source("Birth Register", None, None, Some(1880), None, None).unwrap();
    let cit = a.add_citation(s.id.clone(), e.id.clone(), Some("p.12"), ConfidenceLevel::Primary, None).unwrap();
...
fn citation_update_and_delete() {
    ...
    let e = a.add_event(p.id.clone(), EventType::Death, None, None, None).unwrap();
    let s = a.add_source("Death Records", None, None, None, None, None).unwrap();
    let mut cit = a.add_citation(s.id.clone(), e.id.clone(), None, ConfidenceLevel::Secondary, None).unwrap();
...
fn place_crud() {
    let a = app();
    let pl = a.add_place("London, England", Some(51.5074), Some(-0.1278)).unwrap();
